...
class Encoder:
    ...
    def __call__(self, value, defMode=1, maxChunkSize=0):
        debug.logger & debug.flagEncoder and debug.logger(
            ...
            not defMode and 'in' or '', maxChunkSize, value.__class__.__name__, value.prettyPrint()))
        tagSet = value.getTagSet()
        if len(tagSet) > 1:
        ...
        debug.logger & debug.flagEncoder and debug.logger(
            'using value codec %s chosen by %r' % (concreteEncoder.__class__.__name__, tagSet))
